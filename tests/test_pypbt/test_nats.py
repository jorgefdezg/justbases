...
def test_from_int(value, to_base):
    ...
    return (Nats.convert_to_int(result, to_base) == value and
     result[:1] != [0])
...
def test_carry_in(n,tupla):
    ...
    return (((len(result2) == len(result)) or
    result2[0] == carry_out and 
    result2[1:] == result) and
    (not (len(result2) == len(result)) or result2 == result) and
    len(result2) >= len(result) and
